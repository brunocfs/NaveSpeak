...
export default function VoiceRosterEntry({ username, avatarPath, micStream, moderation }) {
      ...
      {moderation && (
        <details className="group relative ml-auto shrink-0">
          ...
          <div className="absolute right-0 z-10 mt-1 w-56 space-y-1 rounded-lg border border-slate-200 bg-white p-2 text-xs shadow-lg dark:border-slate-700 dark:bg-slate-800">
            {moderation.canMute && (
              <>
                <button className="block w-full rounded px-2 py-1 text-left hover:bg-slate-100 dark:hover:bg-slate-700" onClick={() => moderation.onMute(true, "once")}>
                  Mutar
                </button>
                <button className="block w-full rounded px-2 py-1 text-left hover:bg-slate-100 dark:hover:bg-slate-700" onClick={() => moderation.onMute(true, "lock")}>
                  Mutar e travar 🔒
                </button>
                <button className="block w-full rounded px-2 py-1 text-left hover:bg-slate-100 dark:hover:bg-slate-700" onClick={() => moderation.onMute(false, "lock")}>
                  Destravar áudio
                </button>
              </>
            )}
            {moderation.canDisableMedia && (
              <>
                <button className="block w-full rounded px-2 py-1 text-left hover:bg-slate-100 dark:hover:bg-slate-700" onClick={() => moderation.onDisableMedia(true, "once")}>
                  Desligar mídia
                </button>
                <button className="block w-full rounded px-2 py-1 text-left hover:bg-slate-100 dark:hover:bg-slate-700" onClick={() => moderation.onDisableMedia(true, "lock")}>
                  Desligar mídia e travar 🔒
                </button>
                <button className="block w-full rounded px-2 py-1 text-left hover:bg-slate-100 dark:hover:bg-slate-700" onClick={() => moderation.onDisableMedia(false, "lock")}>
                  Destravar mídia
                </button>
              </>
            )}
            {moderation.canMove && moderation.voiceChannels?.length > 0 && (
              <label className="block px-2 py-1">
                ...
                >
                  <option value="" disabled>Escolher canal</option>
            ...
            {moderation.canDisconnect && (
              <button className="block w-full rounded px-2 py-1 text-left text-red-600 hover:bg-red-50 dark:text-red-400 dark:hover:bg-red-950/30" onClick={moderation.onDisconnect}>
                Desconectar
              </button>
            )}
          </div>
