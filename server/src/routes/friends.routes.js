...
} from "../db/privateMessages.repo.js";
...
router.get("/", async (req, res, next) => {
  try {
    ...
    const lastMessageByPeer = new Map(
      lastMessages.map((r) => [r.peerId, r.lastMessageAt]),
    );
  ...
  async (req, res, next) => {
    try {
      ...
      if (target.id === req.user.internalId) {
        return res
          .status(400)
          .json({ error: "Você não pode adicionar a si mesmo." });
      }
      if (await isBlockedEitherDirection(req.user.internalId, target.id)) {
        return res
          .status(403)
          .json({ error: "Não foi possível enviar a solicitação." });
      }
      ...
      if (result.autoAccepted) {
        ...
        return res
          .status(200)
          .json({ friendship: result.row, autoAccepted: true });
      }
...
async function loadOwnPendingRequest(req, res, next) {
  ...
  if (
    ...
    (request.requester_id !== req.user.internalId &&
      request.addressee_id !== req.user.internalId)
  ) {
...
router.post(
  ...
  loadOwnPendingRequest,
  async (req, res, next) => {
    try {
      if (req.friendRequest.addressee_id !== req.user.internalId) {
        return res
          .status(403)
          .json({ error: "Só o destinatário pode aceitar a solicitação." });
      }
...
router.delete("/:friendId", async (req, res, next) => {
  try {
    ...
    const io = req.app.get("io");
    io?.to(`user:${friend.publicId}`).emit("friend:removed", {
      userId: req.user.id,
    });
...
router.post("/blocks", validateBody(userTagSchema), async (req, res, next) => {
  try {
    ...
    const io = req.app.get("io");
    io?.to(`user:${target.publicId}`).emit("friend:removed", {
      userId: req.user.id,
    });
